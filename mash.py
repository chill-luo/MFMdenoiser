...
paths = os.listdir('data/_SingleImage/raw/')
for path in paths:
    model = UNet_n2n_un(1, 1).cuda()
    model.train()      
    apply_local_shuffling=False 
    with open('data/_SingleImage/fmdd.yaml', 'r') as f:
        config = yaml.safe_load(f)

    upsampler = nn.Upsample(scale_factor=config['std_kernel_size'], mode='nearest')    
    noisy_orig_np =  np.float32(Image.open('data/_SingleImage/raw/' + path))
    clean_orig_np = np.float32(Image.open('data/_SingleImage/gt/' + path))
    ...
    for iter in range(config['num_iterations']):
        ...
        if iter == config['shuffling_iteration']:
            if apply_local_shuffling == True: 
                ...
                tiff.imwrite(f"masks/{path.replace('png', 'tif')}", shuffling_mask)
